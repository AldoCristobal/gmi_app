...
final class RevisionService
{
   ...
   public function show(int $id, array $scope): array
   {
      ...
      if (!$row) return ['ok' => false, 'error' => ['code' => 'NOT_FOUND', 'message' => 'Revisión no visible o inexistente']];
      $docs = $this->repo->docsDeRevision($id);
      $bita = $this->repo->bitacoraList($id, 20, 0);

      $inicial = null;
      $anexos = [];
      foreach ($docs as $d) ((int)$d['is_inicial'] === 1) ? $inicial = $d : $anexos[] = $d;

      return ['ok' => true, 'data' => [
         'revision'   => $row,
         'documentos' => ['inicial' => $inicial, 'anexos' => $anexos],
         'bitacora'   => $bita
      ]];
   }
   ...
   public function crear(array $in): array
   {
      ...
      if (empty($in['evidencia_inicial_token'])) {
         return ['ok' => false, 'error' => ['code' => 'VALIDATION', 'message' => 'evidencia_inicial_token requerido']];
      }

      // Reglas de scope en creación
      if (!empty($in['__user_role']) && $in['__user_role'] === 'auxiliar') {
         if ((int)$in['responsable_id'] !== (int)$in['__user_id'] || (int)$in['area_id'] !== (int)$in['__user_area']) {
            return ['ok' => false, 'error' => ['code' => 'FORBIDDEN', 'message' => 'Fuera de alcance (auxiliar)']];
         }
      }
      if (!empty($in['__user_role']) && $in['__user_role'] === 'supervisor') {
         if ((int)$in['area_id'] !== (int)$in['__user_area']) {
            return ['ok' => false, 'error' => ['code' => 'FORBIDDEN', 'message' => 'Área fuera de alcance (supervisor)']];
         }
      }
      ...
      $id = $this->repo->create([
         ...
         ':estatus'            => 'en_proceso',               // <<-- FORZADO AQUÍ
      ...
      $this->repo->bitacoraAppend(
         $id,
         'subida_doc',
         ['docId' => $docId, 'nombre' => $moved['data']['original'] ?? $moved['data']['filename'], 'is_inicial' => 1],
         (int)($in['__user_id'] ?? 0)
      );

      return ['ok' => true, 'data' => ['id' => $id, 'documento_inicial' => ['id' => $docId]]];
   }
   ...
   public function actualizar(int $id, array $in, array $scope): array
   {
      ...
      $ok = $this->repo->update($id, $this->onlyUpdatable($in, $rev));
      if (!$ok) return ['ok' => false, 'error' => ['code' => 'UPDATE_FAIL']];

      $this->repo->bitacoraAppend(
         $id,
         'actualizacion',
         ['campos' => array_keys($this->onlyUpdatable($in, $rev))],
         (int)($scope['user_id'] ?? 0)
      );
      return ['ok' => true];
   }
   ...
   public function borrar(int $id, array $scope): array
   {
      ...
      $this->repo->delete($id);
      $this->repo->bitacoraAppend($id, 'borrado_revision', null, (int)($scope['user_id'] ?? 0));
      return ['ok' => true];
   }
   ...
   {
      if ($estatus !== 'completa') {
         return ['ok' => false, 'error' => ['code' => 'VALIDATION', 'message' => 'Estatus no permitido']];
      }
      $rev = $this->repo->findVisible($id, $scope);
      if (!$rev) return ['ok' => false, 'error' => ['code' => 'FORBIDDEN']];
   ...
   private function validar(array &$in, bool $isUpdate, ?array $rev = null): true|array
   {
      ...
      if (isset($in['fecha_notificacion']) || isset($in['fecha_vencimiento']) || !$isUpdate) {
         ...
         if ($fn && $fv && $fv < $fn) {
            return ['ok' => false, 'error' => ['code' => 'VALIDATION', 'message' => 'fecha_vencimiento no puede ser menor que fecha_notificacion']];
         }
      }
      // Riesgo válido (estatus ya no se valida aquí para create)
      if (isset($in['riesgo']) && !in_array($in['riesgo'], ['bajo', 'medio', 'alto'], true)) {
         return ['ok' => false, 'error' => ['code' => 'VALIDATION', 'message' => 'riesgo inválido']];
      }
      return true;
   }
   ...
   private function onlyUpdatable(array $in, array $rev): array
   {
      ...
      $out = [];
      foreach ($allowed as $k) if (array_key_exists($k, $in)) $out[$k] = $in[$k];
      return $out;
   }
   ...
   private function consumeTemp(string $token, string $destRelDir): array
   {
      ...
      if (!$matches) return ['ok' => false, 'error' => ['code' => 'VALIDATION', 'message' => 'Token temporal inválido o expirado']];
      ...
      if (!rename($tempPath, $destAbs)) {
         return ['ok' => false, 'error' => ['code' => 'SERVER_ERROR', 'message' => 'No se pudo mover archivo temporal']];
      }

...
final class EmpresaObligacionService
{
   public function __construct(
      private EmpresaObligacionRepository $repo = new EmpresaObligacionRepository(),
      private EmpresaRepository $empRepo = new EmpresaRepository()
   ) {}

   /**
    * Verifica que la empresa sea visible para el scope actual.
    * Devuelve null si OK, o un array ['ok'=>false,...] si está bloqueada.
    */
   private function ensureEmpresaVisible(int $empresaId, array $scope): ?array
   {
      if ($empresaId <= 0) {
         return [
            'ok'    => false,
            'error' => ['code' => 'VALIDATION', 'message' => 'empresa_id inválido'],
         ];
      }

      $emp = $this->empRepo->findVisible($empresaId, $scope);
      if (!$emp) {
         return [
            'ok'    => false,
   ...
   {
      if ($check = $this->ensureEmpresaVisible($empresaId, $scope)) {
         return $check;
      }
      ...
      try {
         ...
         return [
            'ok'    => false,
            'error' => ['code' => 'ERR', 'message' => $e->getMessage()],
         ];
      }
   }
   ...
   public function asignar(array $in): array
   {
      ...
      $in['dia_vencimiento'] = isset($in['dia_vencimiento'])
         ? (is_numeric($in['dia_vencimiento']) ? (int)$in['dia_vencimiento'] : null)
         : null;
      $in['offset_dias']     = isset($in['offset_dias']) ? (int)$in['offset_dias'] : 0;
      $in['activo']          = isset($in['activo']) ? (int)$in['activo'] : 1;
      $in['responsable_id']  = isset($in['responsable_id']) && $in['responsable_id'] !== ''
         ? (int)$in['responsable_id']
         : null;
      $in['area_id']         = isset($in['area_id']) && $in['area_id'] !== ''
         ? (int)$in['area_id']
         : null;
    ...
    */
   public function syncAsignaciones(int $empresaId, array $nuevosIds, array $scope): array
   {
      if ($check = $this->ensureEmpresaVisible($empresaId, $scope)) {
         return $check;
      }
